document.addEventListener("DOMContentLoaded", () => {
  ...
  function updateMetaForSeo(category) {
    const title = category === "All" 
      ? "Daily Pulse & Insights - საინტერესო ფაქტები და რჩევები" 
  ...
  function createPostCard(post) {
    ...
    const shareTitle = encodeURIComponent(post.content ? post.content.substring(0, 60) + "..." : "DailyPulse Post");

    card.innerHTML = `
      <div class="post-media">
        <img src="${post.imageURL}" alt="${post.type} - ${post.content.substring(0, 30)}" itemprop="image" loading="lazy">
        <span class="post-category-tag" data-type="${post.type}" itemprop="articleSection">${post.type}</span>
      </div>
      <div class="post-body">
        <p class="post-text" itemprop="articleBody">${post.content}</p>
        <div class="post-meta">
          <span class="post-date">📅 <time itemprop="datePublished" datetime="${post.dateAdded}">${post.dateAdded}</time></span>
  ...
  function getFilteredPosts(category) {
    return posts.filter(p => category === "All" || p.type === category);
  }
  ...
  function scrollToHashPost() {
    ...
    if (currentCategory !== "All" && targetPost.type !== currentCategory) {
    ...
    const postIndex = filteredPosts.findIndex(p => p.id === targetId);
    if (postIndex !== -1) {
  ...
  tabButtons.forEach(btn => {
    btn.addEventListener("click", () => {
      ...
      initCategory(btn.getAttribute("data-category"));
    });
  });
